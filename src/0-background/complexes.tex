...
We say $\cV = \{V_i\}_{i\in I}$ is a subcover of a cover $\cU = \{U_i\}_{i\in I}$ if $V_i\subseteq U_i$ for all $i\in I$.
The standard proof of the Nerve Theorem, and therefore the Persistent Nerve Lemma, extends directly to pairs of good open covers $(\cU, \cV)$ of pairs $(X, Y)$ such that $\cV$ is a subcover of $\cU$.
% good open covers of spaces $(X, Y)$ covered
% Let $\cU$ be a good open cover of $X$ and let $\cV$ be a good open subcover of $\cU$ that covers $Y\subset X$.
...
Recalling the definition of the strong convexity radius $\varrho_X$ of a metric space $X$ we note that $\cU$ is a good open cover whenever $\varrho_X > \e$.
