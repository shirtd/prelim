...
\begin{definition}[Partial Interleaving of Image Modules]
  ...
  $\Phi(F, G)\in\Hom^\delta(\im~\Gamma,\im~\Lambda)$ is a \textbf{left $\delta$-interleaving of image modules} if there exists some $M\in\Hom^\delta(\S,\V)$ such that $\Gamma[2\delta] = M\circ F$.% $(F, M)$ factors of $\Gamma[2\delta]$ through $\S$ with degree $\delta$.
