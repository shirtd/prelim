...
\begin{proof}
  \[ :( \]
%   Suppose $i : \hom_0(D\setminus B_{\omega})\to\hom_0(D\setminus B_{\omega-3c\delta})$ is \emph{not} injective.
%   So there are $x,x'\in D\setminus B_\omega$ such that $[x]\neq [x']$ but $i[x] = i[x']$ in $\hom_0(D\setminus B_{\omega-3c\delta})$.
%   That is, for every path $\gamma :[0,1]\to D\setminus B_{\omega-3c\delta}$ such that $\gamma(0) = x$ and $\gamma(1) = x'$ there must exist some $t\in [0,1]$ such that $\gamma(t)\in B_\omega$.
%   More
%   % $y\in C_1(D\setminus B_{\omega-3c\delta})$ such that $x = x' + \partial y$
%
%   % The assumptions that $\hom_k(B_{\omega-3c\delta}\hookrightarrow B_\omega)$ is surjective and $\hom_k(B_\omega\hookrightarrow B_{\omega+5c\delta})$ is an isomorphism imply that $\hom_k((D,B_{\omega-3c\delta})\hookrightarrow (D, B_\omega))$ is surjective and $\hom_k((D, B_\omega)\hookrightarrow (D, B_{\omega+5c\delta}))$ is an isomorphism
%   %
%   % $\hom_k(B_{\omega-3c\delta}\hookrightarrow B_\omega)$ is surjective and $\hom_k(B_\omega\hookrightarrow B_{\omega+5c\delta})$
%   % Because $B_{\omega-3c\delta}$ surrounds $D$ in $\X$ we know $D\setminus B_{\omega-3c\delta}$ and $\X\setminus D$ are disjoint, so
%   %
%   % Because $\delta < \varrho_D$ and $P\subset D$ is a $(\delta,2\delta,\omega)$-sublevel sample of $f$, $B_{\omega - 3c\delta}$ surrounds $D$ in $\X$.
%   %
