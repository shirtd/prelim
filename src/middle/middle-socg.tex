...
% \end{lemma}
% \begin{proof}
%   Once again, we assume that $D\setminus \B\subseteq P^\delta$ and $\Q^\of$ surrounds $P^\delta$ in $D$ as in Theorem~\ref{thm:geo_tcc}.
%   Because $\Q^\of \subset \QQ^\of$ it follows that $\QQ^\of$ surrounds $P^\delta$ in $D$ as well.
%   So we can extend $\QQ^\of$ to surround $D$ in $\X$ as $\ext{\QQ^\of} = \QQ^\of\sqcup (D\setminus P^\delta)$.
%
%   For any $x\in \B$ such that $x\notin D\setminus P^\delta$ we have $x\in \B\setminus (D\setminus P^\delta) = \B\cap P^\delta$.
