...
to be the \emph{truncated} $\alpha$ sublevel set and  % of the restricted function $f\rest_{D\setminus B_w}$ for all $\alpha,w\in\R$.
...
This formulation of the TCC states that our approximation by a nested pair of Rips complexes not only covers, but captures the homology of the pair $(D,B_\omega)$ in a specific way.
...
This is done by generalizing our regularity assumptions near $D\setminus B_\omega$ in a way that allows us to interleave persistence modules with static components.% relative to a static submodule.
