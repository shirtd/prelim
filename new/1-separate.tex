...
%
%   If $\hom_k(B\to B')$ is surjective and $\hom_k(B')\cong\hom_k(B'')$ for all $k$ then $\hom_k((A, B)\to (A, B'))$ is surjective for all $A\subseteq D$ such that $B'\subseteq A$ and $\hom_k(A', B')\cong\hom_k(A', B'')$ for all $A'\subset D$ such that $B''\subseteq A'$.
%   % \[ \im~\hom_k((A, B)\to (A, B''))\cong \hom_k(A, B')\]
%   % for any $A\subset D$ such that $
%
...
%   % Note that our assumptions imply that we have the following sequence of inclusions by Lemma~\ref{lem:surround_and_cover}.
%   % % \begin{enumerate}
%   % %   \item $D\setminus B'\subseteq L$,
%   % %   \item $B\cap L \subseteq S\subseteq B'$, and
%   % %   \item $B'\cap L\subseteq S'\subseteq B''$
%   % % \end{enumerate}
%   % % imply that
%   % \[ B\subseteq \ext{S}\subseteq B'\subseteq\ext{S'}\subseteq B''.\]
%   % So have the following sequence of homomorphisms induced by inclusion of pairs
...
% \begin{lemma}
%   If $S$ surrounds $L$ in $D$ and $\ext{S}\subseteq B$ for some subset $B$ of $D$ then $D\setminus B\subseteq L\setminus S$.
% \end{lemma}
% \begin{proof}
%   If $S$ surrounds $L$ in $D$ then $\ext{S} = S\sqcup (D\setminus L)$.
%   So $\ext{S}\subseteq
% \end{proof}
