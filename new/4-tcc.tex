...
We will make the following assumptions about the sublevel-sets of $f$ with respect to constants $\omega\in\R$ and $\gamma\geq \delta > 0$.
...
\begin{small}
    \begin{enumerate}
    \setcounter{enumi}{0}
        \item\textbf{(Surrounds)} $B_{\omega-2c\delta}$ and $B_{\omega + c(\gamma-\delta)}$ are nonempty, compact subsets that surround $\D$ in $\R^d$.
        ...
        \item \textbf{(Surjective Components)} $\hom_0(\D\setminus B_{\omega + c(\gamma-\delta)} \hookrightarrow \D\setminus B_{\omega-2c\delta}^{2\delta})$ is surjective.
        \item \textbf{(Sufficiently Large $\gamma$)} $B_\omega\subseteq B_{\omega-2c\delta}^{\gamma-\delta}\cap\D$.
