...
SEED = 6653831 # np.random.randint(10000000) # 4869361 # 1743801 # 8359430 # 5880992 # 5623320 #
print('seed: %d' % SEED)
np.random.seed(SEED)
...
if __name__ == "__main__":
    ...
    DIR = os.path.join('figures', 'partial5')
    ...
    # P = np.array([[Y[i,j]-1, X[i,j]+1, G[i,j]] for i in range(G.shape[0]) for j in range(G.shape[1]) if np.random.rand()/AMT < G[i,j] ** 2 and i % MOD == 1 and j % MOD == 3]) # *G.max()
    P = np.array([[Y[i,j]-1, X[i,j]+1, G[i,j]] for i in range(G.shape[0]) for j in range(G.shape[1]) if i % MOD == 3 and j % MOD == 0 and G[i,j] >= OMEGA-0.15])# or np.random.rand()/AMT < G[i,j] ** 2)]) # *G.max()
    # P = np.array([[Y[i,j]-1, X[i,j]+1, G[i,j]] for i in range(G.shape[0]) for j in range(G.shape[1]) if np.random.rand() < 1/AMT and G[i,j] >= OMEGA and i % MOD == 0 and j % MOD == 0]) # *G.max()
    ...
    #     if 0 <= i < G.shape[0] and 0 <= j < G.shape[1] and G[i,j] > OMEGA:
    #         _PN.append([x,y,z])
    #
    # PN = np.array(_PN)
