...
OMEGA = CUTS[1]
CONT_ARGS = {   'A_c' : {'scalar' : [OMEGA], 'color' : COLOR['black']},
...
if __name__ == "__main__":
    ...
    DIR = os.path.join('figures', 'surrounding')
    ...
    NOISE_AMT = 0.05
    THRESH = OFF * (1.1*np.sqrt(2 * (2 / N) ** 2) / 2 + NOISE_AMT/2)
    ...
    P = np.array([[Y[i,j]-1, X[i,j]+1, G[i,j]] for i in range(G.shape[0]) for j in range(G.shape[1]) if np.random.rand() / AMT < G[i,j] and G[i,j] > CUTS[0]]) # *G.max()
    # P = np.array([[Y[i,j]-1, X[i,j]+1, G[i,j]] for i in range(G.shape[0]) for j in range(G.shape[1]) if G[i,j] > CUTS[0]]) # *G.max()
    P = np.array([[Y[i,j]-1, X[i,j]+1, G[i,j]] for i in range(G.shape[0]) for j in range(G.shape[1]) if G[i,j] > CUTS[0] and np.random.rand() < 1/AMT]) # *G.max()
    ...
    t = mlab.triangular_mesh(PN[:,0], PN[:,1], Z, T, opacity=0.5, scalars=[1 if i in Qidx else 0 for i in range(len(PN))], color=COLOR['red'])
