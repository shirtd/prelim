...
if __name__ == "__main__":
    ...
    for j, cut in enumerate(self.cuts):
        ...
        ax[1].cla()
        plot_diagram(ax[1], dgm_res.as_list(1), CMAP, THRESH, alpha=0.75, size=10, zorder=1)

        ax[2].cla()
        plot_diagram(ax[2], dgm_rel.as_list(2), CMAP, THRESH, alpha=0.75, size=10, zorder=1)


        ax[1].set_title(r"$\mathrm{H}_1$ restricted (to $B_{%0.1f}$)" % cut)
        ax[2].set_title(r"$\mathrm{H}_2$ relative (to $B_{%0.1f}$)" % cut)

        plt.savefig(os.path.join(dir, 'full-dgm-%d.pdf' % j), dpi=300)
        ...
        for p,q in zip(reversed(dgm_res.get_inf(1)), dgm_rel.get_inf(2)):
            if q.birth - p.birth >= THRESH:
                ...
                plt.pause(0.1)
                plt.savefig(os.path.join(dir, 'dgm-%d_%d.pdf' % (j,i)), dpi=300)
