...
SURF_ARGS = {   'A' : {'min' : CUTS[0], 'max' : CUTS[1],    'color' : COLOR['gray'],   'opacity' : 0.5},#,    'backface_culling' : True},
                'B' : {'min' : CUTS[1], 'max' : CUTS[2],    'color' : COLOR['gray'],    'opacity' : 0.5},
                'C' : {'min' : CUTS[2], 'max' : CUTS[3],    'color' : COLOR['gray'],  'opacity' : 0.5},
                'D' : {'min' : CUTS[3], 'max' : CUTS[4],    'color' : COLOR['gray'],  'opacity' : 0.5}}
...
if __name__ == "__main__":
    ...
    DIR = os.path.join('figures', 'nocover1')
    ...
    NOISE_AMT = 0.05
    THRESH = 1.3*(np.sqrt(2 * (2 / N) ** 2) / 2 + NOISE_AMT)
